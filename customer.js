...
const CURRENT_ORDER_KEY = 'sk_customer_current_order';
...
function loadCurrentOrder() {
  ...
  currentOrder = raw ? JSON.parse(raw) : null;
}
...
function renderCurrentOrder() {
  ...
  title.textContent = currentOrder.status === 'pending' ? 'Pesanan Sedang Diproses' : currentOrder.status === 'preparing' ? 'Pesanan Dipersiapkan' : currentOrder.status === 'ready' ? 'Pesanan Siap' : 'Pesanan Selesai';
  subtitle.textContent = `ID: ${currentOrder.id} · ${currentOrder.timestamp}`;
  itemsEl.innerHTML = currentOrder.items.map(item =>
    `<div class="flex justify-between text-sm text-slate-700"><span>${escapeHtml(item.qty + 'x ' + item.name)}</span><span>Rp ${item.price.toLocaleString('id-ID')}</span></div>`
  ).join('');
  totalEl.textContent = `Rp ${currentOrder.total.toLocaleString('id-ID')}`;
}
